...
When we build progress bar to track progress of any event or task, we often build a linear progress bar. A linear progress bar means that the bar increases proportionally with the progress of the event i.e for 25% progress the bar will show 25% completed. Pretty straightforward right? You might be thinking "Psst! That's how a progress bar works" and you are right but... should you always use it? Is there a better way to build a progress bar?
